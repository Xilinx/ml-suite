...
if [ "$MODEL" == "yolo_v2_224" ]; then
  ...
  INSHAPE_WIDTH=608
  INSHAPE_HEIGHT=608
elif [ "$MODEL" == "yolo_v3_standard_608" ]; then
  NET_DEF=${MLSUITE_ROOT}/models/caffe/yolov3/fp32/yolo_v3_standard_608.prototxt
  NET_DEF_FPGA=${MLSUITE_ROOT}/models/caffe/yolov3/fp32/yolo_v3_standard_608_fpga.prototxt
  FPGAOUTSZ=2048000
  INSHAPE_CHANNELS=3
  INSHAPE_WIDTH=608
  INSHAPE_HEIGHT=608
...
if [ ! -z $CUSTOM_QUANTCFG ]; then
  ...
  if [ "$KCFG" == "v3" ] || [ "$KCFG" == "large" ] || [ "$KCFG" == "med" ] ; then
      ...
      -C "
    echo  -e $RUN_QUANT
    if [ $RUN_QUANT == "yes" ] ; then
      COMPILER_BASE_OPT+=" --usedeephi --quant_cfgfile work/quantize_info.txt  -qz work/quantizer "  
      ...
      echo -e quantize  -model $DUMMY_PTXT -weights $NET_WEIGHTS --output_dir work/  -calib_iter 1 -weights_bit $BITWIDTH -data_bit $BITWIDTH
...
############################  
  ...
  if [ "$TEST" == "streaming_detect_benchmark" ]; then
    ...
    echo -e "not running in benchmark mode" $BASEOPT
  fi 
  if [ "$PERPETUAL" == 1 ]; then 
...
if [ "$TEST" == "yolo_cpp" ]; then
  ...
  #NET_DEF=${MLSUITE_ROOT}/models/darknet/yolov3.prototxt
  #NET_WEIGHTS=${MLSUITE_ROOT}/models/darknet/yolov3.caffemodel
  echo  "running   ccaffe inference for network $NET_DEF and $NET_WEIGHTS"
  ...
  GPU_INFERENCE_OPTS+=" --backend_path ${CAFFE_ROOT}/python"
