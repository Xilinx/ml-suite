...
def draw_boxes(iimage,bboxes,names,colors):
...
#    colors = generate_colors(len(classidset))
    ...
    print "oimage =",oimage
    image.save(os.path.join("out",oimage),quality=90)
    print "Saving new image with bounding boxes drawn as %s" % (os.path.join("out",oimage))
