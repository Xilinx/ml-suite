...
if [ ! -z "$DECENTQ_ERR" ] || [ ! -f $NW_LOG_DIR/quantize_results/deploy.prototxt ] || [ ! -f $NW_LOG_DIR/quantize_results/deploy.caffemodel ] || [ ! -f $NW_LOG_DIR/quantize_results/quantize_info.txt ] ;
then
    echo "Quantizer/decentq error - Failed to run quantizer $ERR" >> nw_status.txt
    ...
    then
	    ...
	    then
	        $python ${MLSUITE_ROOT}/xfdnn/tools/compile/bin/xfdnn_compiler_caffe.py \
                    ...
                    --laodschedule $NW_LOG_DIR/nw.sched \
                    -qz "" 2>&1 |tee $NW_LOG_DIR/compile_out.txt 
                    compile_opt="customreplication + pipelineconvmaxpool + parallelismstrategy + poolingaround + mix + parallelism + Pinc"
...
if [ -z "$STATUS" ];
then
	if [ ! -z "$ERR" ] || [ ! -f ${CMD_FILE} ] || [ ! -f ${CMD_FILE}_quant.json ];
	then
	    echo "compiler error - Failed to run compiler $ERR" >> nw_status.txt
	    echo "check Output Log Directory Path for more details." >> nw_status.txt
        echo "*** Network End" >> nw_status.txt
        exit 1;
	fi
...
then
    ...
    if [ ! -f ${CMD_FILE}_tput.json ];
    then
        echo "compiler Error : Failed to run throughput mode JSON generation" >> nw_status.txt
    	echo "check Output Log Directory Path for more details." >> nw_status.txt
        echo "*** Network End" >> nw_status.txt
        exit 1;
    fi
...
then
    ...
    do
        ...
        then
            ...
            if [ ! -f $NW_LOG_DIR/${array2[$i]}_xfdnn_${NW_NAME}_auto_cut.prototxt ] || [ ! -f $NW_LOG_DIR/${array2[$i]}_xfdnn_${NW_NAME}_auto_train_cut.prototxt ];
            then
    	                echo "HW Error : Failed to run subgraph" >> nw_status.txt
    	                echo "check Output Log Directory Path for more details." >> nw_status.txt
                        echo "*** Network End" >> nw_status.txt
                        exit 1;
    	    fi
            ...
            then
    	        ...
    	        then
                    ...
                    if [ ! -z "$SING_SSD_ERR" ] || [ ! -f $MLSUITE_ROOT/examples/caffe/ssd-detect/test_pic/000022.jpg ];
    	            then
    	                echo "HW Error : Failed to run detection for single image $SING_SSD_ERR" >> nw_status.txt
    	                echo "check Output Log Directory Path for more details." >> nw_status.txt
                        echo "*** Network End" >> nw_status.txt
                        exit 1;
    	            fi
    	        ...
    	        then
                    ...
                    if [ ! -z "$BATCH_SSD_ERR" ];
    	            then
    	                echo "HW Error : Failed to run detection $BATCH_SSD_ERR" >> nw_status.txt
    	                echo "check Output Log Directory Path for more details." >> nw_status.txt
                        echo "*** Network End" >> nw_status.txt
                        exit 1;
    	            fi
                ...
                if [ ! -z "$SING_HW_ERR" ];
    	        then
    	            echo "HW Error : Failed to run hardware $SING_HW_ERR" >> nw_status.txt
    	            echo "check Output Log Directory Path for more details." >> nw_status.txt
                    echo "*** Network End" >> nw_status.txt
                    exit 1;
    	        fi
                ...
                if [ ! -z "$BATCH_HW_ERR" ];
    	        then
    	            echo "HW Error : Failed to run hardware $BATCH_HW_ERR" >> nw_status.txt
    	            echo "check Output Log Directory Path for more details." >> nw_status.txt
                    echo "*** Network End" >> nw_status.txt
                    exit 1;
    	        fi
