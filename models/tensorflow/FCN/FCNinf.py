...
Directories = ["codegeneration","graph","memory","network","optimizations","quantization","version"]
...
for d in Directories:
    sys.path.insert(0, os.path.abspath(cwd+"/../../compile/"+d))

print(sys.path)

import tensor_tools as tt
import dagtools_type as dt
...
tf.flags.DEFINE_string('mode', "fpga", "Mode train/ test/ fpga/ visualize")
...
def inference(image, keep_prob):
    ...
    with tf.variable_scope("inference"):
        ...
        print("shape 3 ", [shape[0], shape[1], shape[2], NUM_OF_CLASSESS])
        print("shape 3 ", image.get_shape())
        #deconv_shape3 = tf.stack([shape[0], shape[1], shape[2], NUM_OF_CLASSESS])  # this messup the memory
        deconv_shape3 = tf.constant([int(shape[0]), int(shape[1]), int(shape[2]), NUM_OF_CLASSESS])
        W_t3 = utils.weight_variable([16, 16, NUM_OF_CLASSESS, deconv_shape2[3].value], name="W_t3")
        b_t3 = utils.bias_variable([NUM_OF_CLASSESS], name="b_t3")
        conv_t3 = utils.conv2d_transpose_strided(fuse_2, W_t3, b_t3, output_shape=deconv_shape3, stride=8)

        annotation_pred = tf.argmax(conv_t3, dimension=3, name="prediction")
        print("name of the last layer ");print(annotation_pred.name)
        print("shape of the last convolution", conv_t3)
...
def main(argv=None):
    ...
    if FLAGS.mode == "fpga":
        ...
        image = tf.placeholder(tf.float32, shape=[1, IMAGE_SIZE, IMAGE_SIZE, 3], name="input_image")
        annotation = tf.placeholder(tf.int32, shape=[1, IMAGE_SIZE, IMAGE_SIZE, 1], name="annotation")
        ...
        with tf.Session(graph=graph) as sess:
            ...
            print("test", image in [ op.name for op in sess.graph.get_operations()])
            ...
            (graph,schedule) = tt.from_tfgraph_to_fpga_code(sess.graph,
    ...
    if FLAGS.mode == "train":
        ...
        with tf.sess.as_default() as sess:
            ...
            alive = sess.run(tensors, feed_dict={image: valid_images, annotation: valid_annotations,
                                                 keep_probability: 1.0})
